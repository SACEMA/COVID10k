...
cumc.p <- cumc %>% select(day, md, hi, hi.hi) %>% mutate(hi = round(hi), hi.hi = round(hi.hi))
names(cumc.p) <- c('Date', 'Median', 'Upper 75%', 'Upper 97.5%')

ggplot(cumc) + 
  geom_line(aes(x = day, y = md)) +
  theme_minimal() + theme(
